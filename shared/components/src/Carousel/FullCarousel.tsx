...
export const S = {
  OverlayContainer: styled.div`
    margin: 0 auto;
    height: 100vh;
    width: ${(props: FullCarouselStyledProps) =>
...
export default (props: FullCarouselComponentProps) => {
  ...
  return (
    <S.OverlayContainer isClicked={props.carousel}>
      <S.ControlCloseBtnWrapper>
        <Image
          priority
          onClick={() => handleChange(props.setShowCarousel)}
      ...
      <S.InnerCarouselContainer>
        <S.CarouselControlBtn
          onClick={() =>
            setImgIndex((prev: number) => (prev <= 1 ? 4 : prev - 1))
          }
        >
          <Image
            priority
        ...
        <Styled.CarouselContainer>
          <S.FullCarouselImageContainer>
            <Card>
              <Image
                priority
                src={`/assets/images/image-product-${imgIndex}.jpg`}
                alt="product-photo"
                fill
              />
            </Card>
        ...
        <S.CarouselControlBtn
          onClick={() =>
            setImgIndex((prev: number) => (prev >= 4 ? 1 : prev + 1))
          }
        >
          <Image
            priority
